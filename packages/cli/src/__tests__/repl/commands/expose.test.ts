...
import { handleExpose } from "../../../repl/commands/expose.js";
import { collectEvents } from "../helpers.js";

describe("handleExpose", () => {
  ...
  it("treats a bare port as the create form", async () => {
    ...
    const events = await collectEvents(handleExpose(box as any, "3000"));
  ...
  it("warns that a server must be detached to survive the command", async () => {
    ...
    const events = await collectEvents(handleExpose(createMockBox() as any, "3000"));
  ...
  it("returns basic-auth credentials when asked", async () => {
    ...
    const events = await collectEvents(handleExpose(box as any, "3000 --basic-auth"));
  ...
  it("lists exposed ports, and with no argument", async () => {
    ...
    for (const args of ["list", ""]) {
      const events = await collectEvents(handleExpose(box as any, args));
      expect(String(events[0]?.message)).toContain("3000");
    }
  });

  it("says so when nothing is exposed", async () => {
    const box = createMockBox();
    box.listPublicURLs.mockResolvedValue({ publicURLs: [] });
    const events = await collectEvents(handleExpose(box as any, "list"));
    expect(events[0]).toEqual({ type: "log", message: "No exposed ports." });
  });

  it("deletes by port", async () => {
    const box = createMockBox();
    await collectEvents(handleExpose(box as any, "delete 3000"));
  ...
  it("rejects a port that is not a number or out of range", async () => {
    ...
    for (const args of ["notaport", "70000", "0"]) {
      const events = await collectEvents(handleExpose(box as any, args));
      expect(String(events[0]?.message)).toContain("Usage: expose");
    }
