...
/**
 * Build the `box` command tree.
 *
 * The standalone binary parses it directly; a host CLI can mount it as a
 * subcommand and supply the token with `setDefaultToken`.
 * @returns the program, not yet parsed.
 */
export function buildBoxProgram(): Command {
  ...
  withCommon(
    ...
    .action(async (path: string, content: string | undefined, flags: Record<string, unknown>) => {
      await runCommand(async () =>
        filesWriteCommand(path, content, { ...globals(flags), ...flags }),
      );
    });
  ...
  withScheduleCommon(
    ...
    runCommand(async () => scheduleGetCommand(id, merged(opts))),
  );
  ...
  withScheduleCommon(
    ...
    runCommand(async () => skillsRemoveCommand(id, merged(opts))),
  );

  withScheduleCommon(skills.command("list").description("List enabled skills")).action(
    async (opts) => runCommand(async () => skillsListCommand(merged(opts))),
  );
  ...
  withBrowserCommon(recordings.command("list").description("List recordings")).action(
    async (opts) => runCommand(async () => recordingListCommand(merged(opts))),
  );

  withBrowserCommon(
    recordings.command("get").argument("<recording-id>").description("Show one recording"),
  ).action(async (id: string, opts) =>
    runCommand(async () => recordingGetCommand(id, merged(opts))),
  );
