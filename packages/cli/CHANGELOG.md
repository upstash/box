...
- Updated dependencies [1a09289]
- Updated dependencies [1a09289]
- Updated dependencies [8fed971]
- Updated dependencies [1a09289]
  - @upstash/box@0.7.6
...
- 7e6feb5: Add `git.createIssue()` and file attachments for issues and pull requests.
...
- 9280c56: Add GPT-6 Astra and Claude Fable 5.1, and correct two model picker entries.
...
- 5a0dff9: Give the CLI parity with the SDK, so an agent driving a terminal can reach everything a program can.
...
- 6fefcb1: Add a non-interactive command surface, so a script, a CI job or a coding agent can drive a box without a terminal.
  ...
  - `box public-url` for public URLs, `box run` for the agent (text on stdout, tool calls on stderr), `box status`, `box use`.
...
- Updated dependencies [b91e466]
- Updated dependencies [677ca08]
- Updated dependencies [b91e466]
- Updated dependencies [e81759e]
  - @upstash/box@0.7.0
...
- 6fd19d5: Add box labels to the CLI: `--label` (repeatable) on `box create`, `--label` filter plus a LABELS column on `box list`, and a `box labels` command group (`add` / `remove` / `list`) to manage labels on a box.
...
- Updated dependencies [6fd19d5]
- Updated dependencies [65b577e]
  - @upstash/box@0.5.4
...
- Updated dependencies [b42b94b]
- Updated dependencies [0ca3415]
  - @upstash/box@0.5.2
