...
class FilesNamespace:
    ...
    def read(
        ...
        *,
        ...
        ``length`` above 8 MiB."""
        return self._box._read_file(path, encoding, offset, length)
    ...
    def stat(self, path: str, *, follow: bool = False) -> FileStat:
...
class Tab:
    ...
    def __init__(self, box: "Box", init: Mapping[str, Any]) -> None:
        self._box = box
    ...
    def content(self) -> BrowserContent:
    ...
    def observe(self, instruction: str, *, model: Optional[str] = None) -> BrowserObserveResult:
    ...
    def live_view_url(self) -> str:
        ...
        no input goes in.
        """
        return self._box._browser_live_view_url(self.id)

    def close(self) -> None:
        """Close this tab."""
        self._box._request("DELETE", f"/v2/box/{self._box.id}/browser/tabs/{self.id}")


class BrowserRecordingHandle:
    """Handle for an in-flight recording returned by ``recordings.start()``."""

    def __init__(self, box: "Box", recording_id: str) -> None:
        self._box = box
        self.id = recording_id

    def stop(self) -> BrowserRecording:
        ...
        current = self._box._recording_get(self.id)
        if current.status != "recording":
            return current
        return self._box._recording_stop()


class BrowserTabNamespace:
    def __init__(self, box: "Box") -> None:
        self._box = box
    ...
    def create(
        ...
        *,
        ...
        """
        return self._box._browser_create_tab(url, wait_until, timeout)


class BrowserRecordingsNamespace:
    ...
    def start(self, *, max_duration_seconds: Optional[int] = None) -> BrowserRecordingHandle:
        """Start capturing. One active recording per box."""
        return self._box._recording_start(max_duration_seconds)

    def stop(self) -> BrowserRecording:
        """Stop the active recording and return its metadata."""
        return self._box._recording_stop()

    def list(self) -> List[BrowserRecording]:
        """List this box's recordings, newest first (auto-paginates)."""
        return self._box._recording_list()

    def get(self, recording_id: str) -> BrowserRecording:
...
class Box(Generic[T]):
    ...
    def _stat_file(self, path: str, follow: bool) -> FileStat:
        resolved = self._resolve_path(path)
    ...
    def _make_dir(self, path: str, parents: bool) -> None:
        resolved = self._resolve_path(path)
        self._request(
            "POST", f"/v2/box/{self.id}/files/mkdir", body={"path": resolved, "parents": parents}
        )

    def _rename_file(self, from_path: str, to_path: str) -> None:
        self._request(
            "POST",
    ...
    def _remove_file(self, path: str, recursive: bool) -> None:
        resolved = self._resolve_path(path)
        self._request(
            "POST",
    ...
    def _browser_create_tab(
        ...
        return Tab(self, resp)

    def _browser_list_tabs(self) -> List[Tab]:
        resp = self._request("GET", f"/v2/box/{self.id}/browser/tabs", timeout=60000)
        return [Tab(self, t) for t in resp.get("tabs") or []]

    def _browser_cdp_url(self) -> str:
        resp = self._request("POST", f"/v2/box/{self.id}/browser/connect", timeout=60000)
        cdp_url = resp.get("cdp_url")
        if not cdp_url:
            raise BoxError("Browser connect did not return a CDP URL")
        return cdp_url

    def _browser_live_view_url(self, tab_id: Optional[str]) -> str:
        resp = self._request(
            "POST",
    ...
    def _recording_start(self, max_duration_seconds: Optional[int]) -> BrowserRecordingHandle:
        body: Dict[str, Any] = {}
        ...
        return BrowserRecordingHandle(self, rec.id)

    def _recording_stop(self) -> BrowserRecording:
        # Stopping flushes the encoder and uploads the HLS artifacts.
        resp = self._request("POST", f"/v2/box/{self.id}/browser/recordings/stop", timeout=180000)
        return self._map_recording(resp)

    def _recording_list(self) -> List[BrowserRecording]:
        ...
        while True:
            ...
            resp = self._request("GET", f"/v2/box/{self.id}/browser/recordings?{urlencode(params)}")
            recordings.extend(self._map_recording(r) for r in resp.get("recordings") or [])
            cursor = resp.get("next_cursor") or None
            if not cursor:
                return recordings

    def _recording_get(self, recording_id: str) -> BrowserRecording:
