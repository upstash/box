...
| `PublicURLListItem` type (JS) | JS types `listPublicURLs()` as a distinct list item (`id`, `created_at`, `basic_auth`, `bearer_token`, `wake_on_request`, no secrets); Python reuses `PublicURL` for create and list, so the list-only fields arrive through `extra="allow"` untyped (Python gap). |
...
- `wake_on_request` on `get_public_url` / `list_public_urls`, and init commands on any box (not just keep-alive): both SDKs dropped the keep-alive guard together.
