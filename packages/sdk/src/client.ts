...
export class Box {
  ...
  private async _executeRun<T>(
    options: RunOptions<T>,
    _attempt: number,
  ): Promise<Run<T | string>> {
  ...
  async logs(options?: { offset?: number; limit?: number }): Promise<LogEntry[]> {
    ...
    const data = await this._request<{ logs: LogEntry[] }>(
      "GET",
      `/v2/box/${this.id}/logs${qs}`,
    )
    return data.logs
  }
  ...
  async listRuns(): Promise<BoxRunData[]> {
    const data = await this._request<{ runs: BoxRunData[] }>(
      "GET",
      `/v2/box/${this.id}/runs`,
    )
    return data.runs
