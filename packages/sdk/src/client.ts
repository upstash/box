...
export class Box {
  ...
  static async create(config: BoxConfig): Promise<Box> {
    ...
    const baseUrl = (config.baseUrl ?? process.env.UPSTASH_BOX_BASE_URL ?? "https://box.api.upstashdev.com").replace(/\/$/, "");
...
function extractSchemaShape(schema: SchemaLike<unknown>): string | null {
  try {
    ...
    if (s.shape && typeof s.shape === "object") {
      const shape = s.shape as Record<string, { _def?: { typeName?: string; type?: { _def?: { typeName?: string } } } }>;
...
function zodTypeToExample(field: unknown): string {
  ...
  switch (typeName) {
    ...
    case "ZodArray": return `[${zodTypeToExample(f._def?.type)}]`;
    default: return "any";
  }
}
