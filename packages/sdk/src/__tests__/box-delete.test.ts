...
describe("Box.deleteAll", () => {
  ...
  it("sends DELETE without body", async () => {
    ...
    await Box.deleteAll({ apiKey: TEST_CONFIG.apiKey, baseUrl: TEST_CONFIG.baseUrl });
  ...
  it("throws when apiKey is missing", async () => {
    await expect(Box.deleteAll()).rejects.toThrow("apiKey is required");
  });
  ...
  it("throws on API error", async () => {
    ...
    await expect(
      Box.deleteAll({ apiKey: TEST_CONFIG.apiKey, baseUrl: TEST_CONFIG.baseUrl }),
